...
import actionlib # to implement server- client concept
...
from sensor_msgs.msg import LaserScan # Return values of laser scan sensor
from nav_msgs.msg import Path, Odometry
from nav_msgs.srv import GetPlan # Plan message as array of waypoints
...
class turtlebot:
    def __init__(self): # Constructor of class to initialise all the subscribers and publisher and the variables.
        number = rospy.get_param('/harry_potter/number')
        rospy.init_node('harry_potter') # ROS node initialisation
        rospy.Subscriber('/goals', PointArray, self.goals_callback)
        ...
        # parameters of the move base node are modified as follows:
        rospy.set_param('/robot'+str(number)+'/move_base/local_costmap/inflation_layer/inflation_radius', 0.35)
        rospy.set_param('/robot'+str(number)+'/move_base/local_costmap/inflation_layer/cost_scaling_factor', 2.5)
        rospy.set_param('/robot'+str(number)+'/move_base/global_costmap/inflation_layer/inflation_radius', 0.6)
        rospy.set_param('/robot'+str(number)+'/move_base/global_costmap/inflation_layer/cost_scaling_factor', 2.5)

        rospy.set_param('/robot'+str(number)+'/move_base/DWAPlannerROS/path_distance_bias', 26)
    ...
    def laser_callback(self, scan_msg): # Call back function for /scan topic
        ...
        self.robfront =  min(min(self.laser_msg.ranges[0:25]),min(self.laser_msg.ranges[335:360]), 7)
        self.robfleft =  min(min(self.laser_msg.ranges[26:60]), 7)
        self.robfright =  min(min(self.laser_msg.ranges[299:334]), 7)

        self.robfront2 =  min(min(self.laser_msg.ranges[0:45]),min(self.laser_msg.ranges[315:360]), 7)
        self.robfleft2 =  min(min(self.laser_msg.ranges[46:60]), 7)
        self.robfright2 =  min(min(self.laser_msg.ranges[299:314]), 7)
    ...
    def turtlebot_pose_callback(self, pose_msg):# call back for position and orientation of robot
    ...
    def get_next_goal(self, array_goals):   # To get the next nearest goal from after the bot reaches the current goal.
        ...
        # Sorting is performed based on reward to all goals from robot's current position in ascending order.
        goal = temp_goals[0]
    ...
    def service_plan_call(self,goal):# Service call to get plan from move base
        self.waypoints_temp = []
        start = PoseStamped()
        start.header.seq = 0
    ...
    def distance_move(self,distance):# Move until 'distance' is travelled only when no obstacles are present in the path.
        p1 = self.turtlebot_position
        while self.robfront > self.obs_dist:
            self.linear_move()
    ...
    def move_bot(self):
        self.obs_dist = 0.2# distance to maintain with obstacle
        while(len(self.goal_arr) != 0): # To reach the goals in goal_arr and then exit without error
            ...
            while True:
                ...
                if(self.MB_client.get_state() == 3 or self.dist < 0.1 ):
                    rospy.sleep(1)
                    print "Reached Goal"   # Distance to goal 0.1, considered as reached.Because the diameter of bot is approximately 0.17
                    self.reward += goal.reward
                    print "Accumulated rewards:", self.reward
                    break
                if (t2-t3)> 120: # t2-t3 tracks the overall goal active time
                ...
                if self.MB_client.get_state() == 4 or (self.linear_vel == 0.0 and self.ang_vel == 0.0 and (t2-t1) > 15):
                    #t2-t1 is the time counter that limits goal handling time of move base.
                    rospy.sleep(3)
                    if self.linear_vel == 0.0 and self.ang_vel == 0.0:
                        ...
                        self.service_plan_call(goal) # Get only the plan from move base but goal is not sent to the move base.
                        self.waypoints = np.copy(self.waypoints_temp)

                        try:
                            self.rob_turn(self.steering_angle(self.turtlebot_position,self.waypoints[10])) #waypoint at distance 0.25m chosen
                            self.init_condition()
                            while 1:
                                if self.distance(self.waypoints[10], self.turtlebot_position) < 0.05:
                                    break

                                if self.robfront > self.obs_dist:
                                    self.linear_move()
                                else:# while driving to the waypoint, if there is an obstacle we move to the free region and try to move 1m randomly and then replan using move base.
                                    if self.robfront <= self.obs_dist:

                                        if self.robfleft >= 0.5:   #  Check if no obstacle on left side
                                        ...
                                        elif self.robfright >= 0.5:    # Check if no obstacle on right side
                                        ...
                                        if self.robfront > self.obs_dist:
                                            ...
                                            continue
                                        self.init_condition()# After moving some distance steer towards waypoint and then send goal to move base
                                        self.rob_turn(self.steering_angle(self.turtlebot_position,self.waypoints[10]))
                        except:
                            a=0
                ...
                if goal.reward >= 90:
                    t4 = rospy.get_time()
                    #t4-t5 is the time counter to exit from 100 rewards goals.

                    if 1:
                        print "Entering manual control-2"
                        self.MB_client.cancel_goal()
                        rospy.sleep(1)
                        self.init_condition()
                        ...
                        if len(self.waypoints) > 20:
                            while len(self.waypoints) > 10:
                                t5 = rospy.get_time()
                                print "follow waypoints"
                                self.rob_turn(self.steering_angle(self.turtlebot_position,self.waypoints[i])) #waypoint at distance 0.25m chosen
                                self.init_condition()
                                if (t5 - t4) > 60:
                                    break
                                while 1:
                                    ...
                                    if self.distance(self.waypoints[i], self.turtlebot_position) < 0.01:
                                        break

                                    if self.robfront2 > self.obs_dist:
                                        self.linear_move()
                                    else:# while driving to the waypoint, if there is an obstacle we move to the free region and try to move 1m randomly and then replan using move base.
                                        if self.robfront2 <= self.obs_dist:
                                            if self.robfleft2 >= 0.5:   #  Check if no obstacle on left side

                                                self.init_condition()
                                                while (self.robfront2 <= self.obs_dist):
                                            ...
                                            elif self.robfright2 >= 0.5:    # Check if no obstacle on right side
...
if __name__ == '__main__':
    ...
    t.move_bot() # To start the path planning to reach the skipped goals.
    while not rospy.is_shutdown(): # After reaching all the goals, control waits here until the user stops the node using ctrl+c
        rospy.sleep(0.5)
